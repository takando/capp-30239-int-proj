...
d3.csv("https://docs.google.com/spreadsheets/d/1BoR_iikHS0ghwFUn-vpohrnw_OD7rOIdOoNSfo3gpAw/export?format=csv&gid=1687591646").then(
 function(data){
  ...
  var nest = d3.nest()
    ...
    // age axis2
    var ax_age_svg2 = svg.append( 'g' )
        .attr('class', 'axis age')
        .attr('transform', 'translate(' + (cx+gutter/2+300+10) + ',0)')
        .call( ax_age_l );

    ax_age_svg2.append( 'text' )
        .attr( 'dy', '.32em' )
        .text('Age');

    ax_age_svg2.selectAll( 'text' )
        .attr('x', -gutter/2-10)
        .style('text-anchor', 'middle');

    var ax_age_r2 = d3.axisRight()
        .scale(s_age)
        .tickFormat( d => '' );
    ...
    var slider = svg.append("g")
      .attr("class", "slider")
      .attr("transform", "translate(" +  (cx-350) +  "," + (pyramid_h+85) + ")");
    ...
    var initialGraph = function(country, year){
        ...
        var pyramid1 = svg.selectAll(".pyramid1")
        ...
        var pyramid2 = svg.selectAll(".pyramid2")
            .data(selectyear, function(d){
              return d ? d.key : this.key;
            })
            .enter()
        ...
        var bar1 = pyramid1.selectAll( '.bar1' ).data(function(d) { return d.values; }  );
        var bar2 = pyramid2.selectAll( '.bar2' ).data(function(d) { return d.values; }  );
    ...
    var other_country = d3.select("#current_country2")
                          .attr("value")
    ...
    var other_country = d3.select("#current_country1")
                          .attr("value")
    ...
    d3.select("#current_country1")
      .attr("value", country1)
    
    d3.select("#current_country2")
      .attr("value", country2)
...
function updateyear(h) {
  ...
  var current_country1 = d3.select("#current_country1")
                          .attr("value")

  var current_country2 = d3.select("#current_country2")
                          .attr("value")
